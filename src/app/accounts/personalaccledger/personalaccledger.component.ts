...
export class PersonalaccledgerComponent implements OnInit {
  ...
  getDistinctPersonalAccs(){
    const urldata = "fromdt="+this.finanyr.fromdt+"&todt="+this.finanyr.todt;
    this._rest.getData("accounts.php", "getDistinctPersonalAccs", urldata)
      .subscribe(Response=>{
        this.personalaccs = Response && Response["data"]? Response["data"]: null;
        this.personalaccs = this.personalaccs.sort(this._global.sortArr("name"));
      },error=>{
        this.personalaccs=[];
      })
  }
  ...
  fetchPersonalAccountDetails(acc){
    ...
    this.getExpendituresOfPersonalAcc(acc.personalaccid).then(expenditures=>{
      // console.log(expenditures);
      this.getReceiptsOfPersonalAcc(acc.personalaccid).then(receipts=>{
        // console.log(receipts);
        this.filterData();
      }).catch(errrec=>{
        alert("Error fetching cash receipts, kindly refresh or try again later");
      })
    }).catch(errexp=>{
  ...
  getExpendituresOfPersonalAcc(personalaccid){
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      _this._rest.getData("accounts.php", "getExpendituresOfPersonalAcc", urldata)
        .subscribe(Response=>{
          _this.allexpenditures= Response && Response["data"] ? Response["data"] : null;
  ...
  getReceiptsOfPersonalAcc(personalaccid){
    ...
    const promise = new Promise((resolve, reject) => {
      const urldata = `fromdt=${_this.finanyr.fromdt}&todt=${_this.finanyr.todt}&personalaccid=${personalaccid}`;
      _this._rest.getData("accounts.php", "getReceiptsOfPersonalAcc", urldata)
        .subscribe(Response=>{
          _this.allreceipts= Response && Response["data"] ? Response["data"] : null;
  ...
  filterData(){
    ...
    let tmpbalance = parseFloat(this.ledgerhist[0].debit)>0?parseFloat(this.ledgerhist[0].debit):parseFloat(this.ledgerhist[0].credit);
    this.ledgerhist[0].balance = tmpbalance;
    for (let k=1;k<this.ledgerhist.length;k++) {
      this.ledgerhist[k].balance = tmpbalance + parseFloat(this.ledgerhist[k].debit) - parseFloat(this.ledgerhist[k].credit);
