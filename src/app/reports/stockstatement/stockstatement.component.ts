...
export class StockstatementComponent implements OnInit {
  ...
  filterData(data){
    ...
    const promise = new Promise((resolve, reject) => {
      for (let i = 0; i < data.length; i++) {
        ...
        if (i == 0) {
          if(!this.prevrecfromfilter || (this.prevrecfromfilter && this.prevrecfromfilter.length<=0)){
            console.log("If",data[0])
            tmpObj.openingstock = data[i].closingstock ? data[i].closingstock:parseFloat(data[i].quantity);
            tmpObj.quantity = 0;
            tmpObj.closingstock = data[i].closingstock ? data[i].closingstock:parseFloat(data[i].quantity);
          }
          else{
            console.log("Else")
            tmpObj.openingstock = parseFloat(this.prevrecfromfilter.closingstock);
            tmpObj.closingstock = tmpObj.inout === CONSTANTS.IN ? parseFloat(tmpObj.openingstock) + parseFloat(tmpObj.quantity) : parseFloat(tmpObj.openingstock) - parseFloat(tmpObj.quantity);
          }
        } else {
  ...
  autoFillDt(){
    ...
    this.filterfromdt = this._global.getAutofillFormattedDt(this.filterfromdt);
  ...
  /* filterStockStatementFromToDate(){
    this.filtereddt.fromdt = new Date(moment(this.filterfromdt, "DD-MM-YYYY").format("MM-DD-YYYY")).getTime();
    this.filtereddt.todt = new Date(moment(this.filtertodt, "DD-MM-YYYY").format("MM-DD-YYYY")).getTime();
    ...
    this.getFromToStockHistory(this.selectedmat,this.filtereddt.fromdt,this.filtereddt.todt); 
  }

  getFromToStockHistory(stock, fromdt, todt){
    this.isLoading = true;
    let geturl = `stockid=${stock.stockid}&fromdt=${fromdt}&todt=${todt}`;
    this._rest.getData("stock.php", "getFromToStockHistory", geturl)
      .subscribe(Response=>{
        const data = Response && Response["data"]? Response["data"]: null;
        this.prevrecfromfilter = Response && Response["openstk"]? Response["openstk"]:null;
